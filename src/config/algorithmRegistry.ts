...
export const algorithmRegistry: AlgorithmConfig[] = [
  ...
  // Module 3: String Matching — KMP Algorithm
  {
    ...
    pseudocode: [
      ...
      '  m = length(pattern); lps = array of zeros of size m',
      '  len = 0, i = 1  // len: length of previous longest prefix-suffix',
      '  while i < m:  // Construct LPS table for pattern',
      '    if pattern[i] == pattern[len]:',
      '      len++; lps[i] = len; i++  // Prefix match extended',
      '    else:',
      '      if len != 0: len = lps[len - 1]  // Fallback to previous prefix-suffix',
      '      else: lps[i] = 0; i++  // No prefix match, advance index',
      '  return lps',
      '',
      'function KMP_Search(text, pattern):',
      '  lps = computeLPS(pattern)  // Phase 1: Precompute LPS array',
      '  i = 0, j = 0  // Phase 2: i tracks text, j tracks pattern',
      '  while i < length(text):  // Zero-backtracking scan of text',
      '    if pattern[j] == text[i]: i++, j++  // Character match: advance both',
